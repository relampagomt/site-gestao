...
function decorate(row) {
  ...
  return { ...row, status, _valor: valor, _pago: pago, _aberto: aberto, _venc: venc };
}
...
export default function ContasPagarTab() {
  ...
  const openEdit = (row) => {
    ...
    setForm({
      ...
      dataPagamentoMask: isoToMask(row.dataPagamento || ''),
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (normalizeStatus(status) === 'Pago') {
        valorPago = valor;
        if (!dataPagamento) dataPagamento = vencimento;
      }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Gerencie despesas</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="p-3 rounded-lg border"><div className="text-xs opacity-70">Saídas</div><div className="text-lg font-semibold">{BRL(kpis.total)}</div></div>
            <div className="p-3 rounded-lg border"><div className="text-xs opacity-70">Total Pago</div><div className="text-lg font-semibold">{BRL(kpis.pago)}</div></div>
          ...
          <div className="overflow-auto rounded-lg border">
            <Table>
              ...
              <TableBody>
                ...
                {!loading && filtered.length===0 && (<TableRow><TableCell colSpan={7} className="text-center py-6">Sem registros</TableCell></TableRow>)}
              </TableBody>
            </Table>
          </div>
        </CardContent>
          ...
          <form onSubmit={submit} className="max-h-[80vh] flex flex-col">
            ...
            <div className="px-6 pb-6 overflow-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><Label>Documento</Label><Input value={form.documento} onChange={(e)=>setForm(f=>({...f,documento:e.target.value}))}/></div>
                ...
                <div><Label>Data Pagamento</Label><Input placeholder="dd/mm/aaaa" value={form.dataPagamentoMask} onChange={(e)=>setForm(f=>({...f,dataPagamentoMask:maskDateBR(e.target.value)}))}/></div>
