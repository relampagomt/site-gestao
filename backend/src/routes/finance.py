...
from src.middleware.auth_middleware import roles_allowed  # continua usando o seu
...
# ====== CORS local (como no antigo, opcional) ======
_ALLOWED_ORIGINS = set([o.strip() for o in (os.getenv("FRONTEND_URL") or "").split(",") if o.strip()])
...
def _month_bounds(ym: str) -> Optional[tuple]:
    if not (isinstance(ym, str) and len(ym) == 7 and ym[4] == "-"): return None
    y, m = ym.split("-")
    try:
        y, m = int(y), int(m)
...
# ====== Firestore I/O antigo (com fallback JSON) ======
_fs_client = None
...
def _get_firestore():
    ...
    try:
        ...
        if b64:
            ...
            if not info: raise RuntimeError("FIREBASE_CREDENTIALS_B64 inválido")
            ...
            _fs_ready = True; _fs_client = _client; return _fs_client
        ...
        _fs_ready = False; _fs_client = None; return None

def _json_load() -> List[Dict[str, Any]]:
    try:
        if not os.path.exists(_JSON_PATH): return []
        with open(_JSON_PATH,"r",encoding="utf-8") as f: data = json.load(f)
        return data if isinstance(data,list) else []
    except Exception as e:
        log.exception("[finance] erro lendo JSON: %s", e); return []

def _json_save(items: List[Dict[str, Any]]) -> None:
    try:
        with open(_JSON_PATH,"w",encoding="utf-8") as f: json.dump(items,f,ensure_ascii=False)
    except Exception as e:
...
def _fs_create(doc: Dict[str, Any]) -> Dict[str, Any]:
    client = _get_firestore()
    if not client:
        items = _json_load(); items.append(doc); _json_save(items); return doc
    try:
    ...
    if not client:
        ...
        for i,it in enumerate(items):
            if str(it.get("id")) == str(txid):
                items[i].update(upd); _json_save(items); return _doc_to_json(items[i])
        return None
    try:
        ...
        log.exception("[finance][fs_update] falha: %s", e); return None

def _fs_delete(txid: str) -> bool:
    client = _get_firestore()
    if not client:
        items = _json_load(); new_items = [x for x in items if str(x.get("id")) != str(txid)]
        _json_save(new_items); return True
    try:
        ref = client.collection(_COLL).document(txid)
        if not ref.get().exists: return True
...
def update_transaction(txid):
    try:
        ...
        if "type" in data:
            ...
            if t in _VALID_TYPES: upd["type"] = t
        if "date" in data:
            d = _iso_date_only(data["date"])
            if d: upd["date"] = d
        if "amount" in data: upd["amount"] = abs(_to_number(data["amount"]))
...
# ====== ADAPTADORES NOVOS (mantém a lógica antiga) ======
def _map_pagar(payload: Dict[str,Any]) -> Dict[str,Any]:
    return {
        "type": "saida",
        "date": _iso_date_only(payload.get("vencimento") or payload.get("date") or payload.get("data")),
        "amount": abs(_to_number(payload.get("valor") if payload.get("valor") is not None else payload.get("amount"))),
        "category": (payload.get("categoria") or payload.get("category") or "conta_pagar").strip(),
        "notes": (payload.get("descricao") or payload.get("notes") or "").strip(),
        "action_id": (payload.get("action_id") or "").strip() or None,
    }

def _map_receber(payload: Dict[str,Any]) -> Dict[str,Any]:
    notes = payload.get("descricao") or payload.get("cliente") or payload.get("notaFiscal") or payload.get("notes") or ""
    return {
        "type": "entrada",
        "date": _iso_date_only(payload.get("vencimento") or payload.get("dataEmissao") or payload.get("date") or payload.get("data")),
        "amount": abs(_to_number(payload.get("valor") if payload.get("valor") is not None else payload.get("amount"))),
        "category": (payload.get("categoria") or payload.get("category") or "conta_receber").strip(),
        "notes": notes.strip(),
        "action_id": (payload.get("action_id") or "").strip() or None,
    }
...
@roles_allowed('admin')
def listar_pagar():
    args = request.args.to_dict(flat=True); args["type"] = "saida"
...
@roles_allowed('admin')
def listar_receber():
    args = request.args.to_dict(flat=True); args["type"] = "entrada"
    with finance_bp.test_request_context(query_string=args):
        return list_transactions()
...
def criar_pagar():
    ...
    if not mapped.get("date"): return jsonify({"message":"vencimento/date inválido (YYYY-MM-DD)"}), 400
    request._cached_json = mapped  # reaproveita a rota antiga
...
@roles_allowed('admin')
def atualizar_pagar(txid: str):
    data = request.get_json(silent=True) or {}
    upd = _map_pagar(data)
    upd = {k:v for k,v in upd.items() if k in data or k=="type"}
    if "date" in upd and not upd["date"]: upd.pop("date")
    if not upd: return jsonify({"message":"Nada para atualizar"}), 400
    out = _fs_update(txid, upd)
    if not out: return jsonify({"message":"Transação não encontrada"}), 404
    return jsonify(_doc_to_json(out)), 200

@finance_bp.route("/contas-receber/<txid>", methods=["PUT","PATCH"])
@roles_allowed('admin')
def atualizar_receber(txid: str):
    data = request.get_json(silent=True) or {}
...
@roles_allowed('admin')
def deletar_pagar(txid: str):
    _fs_delete(txid); return ("",204)

@finance_bp.route("/contas-receber/<txid>", methods=["DELETE"])
@roles_allowed('admin')
def deletar_receber(txid: str):
    _fs_delete(txid); return ("",204)
