...
def _norm_status(v):
    ...
    if s.lower() == "pago": return "Pago"
    if s.lower() == "pendente": return "Pendente"
    if s.lower() == "cancelado": return "Cancelado"
    # default
    return "Pendente"
...
def _to_float(v, default=0.0):
    try:
        if isinstance(v, str):
            # troca vírgula por ponto para casos "1.234,56"
...
def _next_id(items):
    # ids como string mantendo compatibilidade
    mx = 0
    for it in items:
        try:
...
def _apply_aliases(payload):
    ...
    - action_text <= action_text | pay_date | payDate
    ...
    # se não for data válida, manter texto original (retrocompat)
    ...
    # juros (string); e interest_rate numérico auxiliar
    ...
    try:
        # tenta converter material_value para float auxiliar
        iv = str(material_value).replace(".", "").replace(",", ".")
        interest_rate_num = float(iv)
    except Exception:
        # se vier interest_rate separado, usa ele
        try:
    ...
    data = {
        "date": date_iso,                        # obrigatório, validado no handler
        ...
        # retrocompat: *_id podem vir, mantemos no registro sem uso no front novo
    ...
    # auxiliar: interest_rate numérico quando fizer sentido
...
def patch_transaction(id):
    ...
    # Aplicamos aliases mas permitimos parciais (PATCH)
    partial = _apply_aliases(payload)

    # Se não veio date no PATCH, mantemos o atual
    if not payload.get("date") and not payload.get("due_date") and not payload.get("dueDate"):
        partial["date"] = current.get("date")

    # Merge e normalizações importantes
    merged = {
        **current,
        **{k: v for k, v in partial.items() if v is not None}
    }
    # Normaliza type/status/amount mesmo em parciais
