...
import { Search, Edit, Trash2, TrendingUp, ArrowDownCircle, ArrowUpCircle, Plus } from 'lucide-react';
...
const emptyForm = {
  type: 'entrada',
  dueDateBr: '',       // Data de vencimento
  payDateBr: '',       // Data de pagamento
  amount: '',
  category: '',
  ...
  interestRate: '',
  notes: '',
  // campos de compat (itens antigos / payload)
  action_id: '',
  client_id: '',
  material_id: '',
};
...
const Finance = () => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((t) =>
        [
          ...
          t.client_text, t.material_text,                 // meio de pagamento / juros
          t.status,
          // datas em BR também entram na busca
          isoToBR(String(t.date || '').slice(0,10)),      // vencimento
          isoToBR(String(t.action_text || '').slice(0,10))// pagamento (armazenado em action_text)
        ]
          .filter(Boolean)
  ...
  const categoryChart = useMemo(() => {
    const map = {};
    filtered.forEach((t) => {
      if (t.type === 'entrada') return; // não conta entradas
  ...
  const monthlyChart = useMemo(() => {
    ...
    filtered.forEach((t) => {
      ...
      if (t.type === 'entrada') months[month].entrada += v;
      if (t.type === 'saida')   months[month].saida   += v;
      if (t.type === 'despesa') months[month].despesa += v;
    });
  ...
  const exportData = useMemo(()=> filtered.map((t)=>({
    vencimento: isoToBR(String(t.date||'').slice(0,10)) || '',
    pagamento:  isoToBR(String(t.action_text||'').slice(0,10)) || '',
    tipo: t.type || '',
  ...
  const pdfOptions = {
    ...
    columnStyles: { 0:{cellWidth:26},1:{cellWidth:26},2:{cellWidth:20},3:{cellWidth:34},4:{cellWidth:22},5:{cellWidth:36},6:{cellWidth:22},7:{cellWidth:26},8:{cellWidth:70} },
    footerContent:`Totais do período: Entradas: ${BRL(totalEntrada)} | Saídas: ${BRL(totalSaida)} | Saldo: ${BRL(saldo)}`
  };

  // handlers
  const onChange = (e) => setForm((f)=>({...f,[e.target.name]: e.target.value}));
  const onDueChange = (e) => setForm((f)=>({...f, dueDateBr: maskBR(e.target.value)}));
  const onPayChange = (e) => setForm((f)=>({...f, payDateBr: maskBR(e.target.value)}));
  ...
  const openEdit = (tx) => {
    ...
    setForm({
      ...
      dueDateBr: isoToBR(String(tx.date || '').slice(0,10)) || '',
      payDateBr: isoToBR(String(tx.action_text || '').slice(0,10)) || '',
  ...
  const submit = async (e) => {
    ...
    try {
      const dueISO = brToISO(form.dueDateBr);
      if (!isYMD(dueISO)) { alert('Vencimento inválido. Use DD/MM/AAAA.'); setSaving(false); return; }

      const payISO = form.payDateBr ? brToISO(form.payDateBr) : '';
      if (form.payDateBr && !isYMD(payISO)) { alert('Pagamento inválido. Use DD/MM/AAAA.'); setSaving(false); return; }

      const payload = {
        type: form.type,
        date: dueISO,                                  // vencimento
        amount: Number(form.amount || 0),
        category: form.category || '',
        status: form.status || 'Pendente',
        notes: form.notes || '',
        action_text: payISO || '',                     // pagamento
        client_text: form.paymentMethod || '',         // meio de pagamento
        material_text: String(form.interestRate || ''),// taxa de juros
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={categoryChart} cx="50%" cy="50%" labelLine={false}
                  ...
                  {categoryChart.map((entry,i)=>(<Cell key={i} fill={colorForCategory(entry.name)} />))}
                </Pie>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={monthlyChart} margin={{ top:10, right:10, left:0, bottom:20 }} barCategoryGap="20%" barGap={6}>
                ...
                <XAxis dataKey="month" />
                <YAxis tickFormatter={BRL_COMPACT} domain={[0,(max)=>(max||0)*1.15+1]} allowDecimals={false}/>
                <Tooltip formatter={(v)=>BRL(v)} />
                <Legend />
      ...
      <Card className="mb-4">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center">
            ...
            <div className="w-full">
              <Button className="w-full gap-2" onClick={openCreate}><Plus className="size-4" />Novo Lançamento</Button>
            </div>
          </div>
        </CardContent>
      </Card>
                  ...
                  filtered.map((t) => {
                    ...
                    return (
                      <TableRow key={t.id}>
                        ...
                        <TableCell className="text-center">{isoToBR(String(t.action_text || '').slice(0,10)) || '—'}</TableCell>
                        ...
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <select className="border rounded-md px-2 py-1 text-xs bg-background" value={t.status || 'Pendente'} onChange={(e)=>updateStatus(t, e.target.value)}>
                              <option>Pago</option>
          ...
          <form onSubmit={submit} className="max-h-[80vh] flex flex-col">
            <div className="p-6 pb-2">
              <DialogHeader className="pb-2">
                ...
                <DialogDescription>
                  Defina o <strong>Vencimento</strong>, opcionalmente a <strong>Data de Pagamento</strong>, o <strong>Meio de Pagamento</strong> e a <strong>Taxa de Juros</strong>.
                </DialogDescription>
              </DialogHeader>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {/* datas alinhadas lado a lado */}
                <div>
                  <Label>Data de Vencimento</Label>
                  <Input name="dueDateBr" placeholder="DD/MM/AAAA" inputMode="numeric" value={form.dueDateBr} onChange={onDueChange} required />
                </div>
                <div>
                  <Label>Data de Pagamento</Label>
                  <Input name="payDateBr" placeholder="DD/MM/AAAA" inputMode="numeric" value={form.payDateBr} onChange={onPayChange} />
                </div>
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <Input name="interestRate" type="number" step="0.01" value={form.interestRate} onChange={onChange} placeholder="Ex.: 2.50" />
                  </div>
