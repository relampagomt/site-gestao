...
export default function ContasReceberTab() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return items.filter((it) =>
      [it.cliente, it.notaFiscal, it.notes, it.category, it.date, toBR(it.date)]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(t)
        ...
        <div className="card border border-base-200">
          <div className="card-body p-0">
            ...
            <div className="p-3 text-xs text-gray-500">
              Total: {filtered.length} conta(s) | A Receber: {dinheiro(totals.valor)} |
              Recebido Líq.: {dinheiro(totals.liq)} | Em Aberto:{" "}
              {dinheiro(totals.saldo)}
            </div>
          </div>
        </div>
